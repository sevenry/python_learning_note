...
class DefaultErrorHandler(urllib.request.HTTPDefaultErrorHandler):
    '''@xxx:bvb: Where is this used? should it be registered somewhere with urllib.request?
...
def http_request(url, user_agent=USER_AGENT, retry=4):
    """Retrieve the contents referenced by the URL using urllib.request.
...
def http_auth_request(url, host, user, passwd, user_agent=USER_AGENT):
    """Call an HTTP server with authorization credentials using urllib.request.
    """
    if DEBUG: httplib.HTTPConnection.debuglevel = 1

    # Hook up handler/opener to urllib.request
...
class DeliciousAPI:
    ...
    def __init__(self, user, passwd, codec='iso-8859-1', api_request=dlcs_api_request, xml_parser=dlcs_parse_xml):
        ...
        see ``urllib.request.openurl`` for more info.
        """
        assert user != ""
        self.user = user
        self.passwd = passwd
    ...
    def request_raw(self, path, **params):
        ...
        ``urllib.request.openurl`` documentation.
        """
